...
class myThread (threading.Thread):
  ...
  def run(self):
    ...
    with open("{}/hsfetcher/Memory_Dumps/{}H-{}.str".format(sys.path[0], self.extraction_datetime, self.pid), "r") as self.strings_file:
  ...
  def dump_memory(self, pid):
    self.process_manager = subprocess.Popen(["{}/hsfetcher/process_dumper.sh".format(sys.path[0]), pid], stdout=subprocess.PIPE, universal_newlines=True)
  ...
  def calc_onion_link(self, pkey):
    ...
    self.process_manager = subprocess.Popen(["{}/hsfetcher/onion-link-calc.sh".format(sys.path[0]), pkey], stdout=subprocess.PIPE, universal_newlines=True)
    self.output, self._err = self.process_manager.communicate()
...
def fetchme():
  ...
  if Path("{}/hsfetcher/Database/hidden_services.db".format(sys.path[0])).is_file():
